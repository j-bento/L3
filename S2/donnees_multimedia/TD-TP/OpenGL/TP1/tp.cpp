...
void setUnitSphere( Mesh & o_mesh, int nX=20, int nY=20 )
{
    ...
    for (const Vec3& sommet : o_mesh.vertices) { //pour chaque sommet on le divise par la norme
        o_mesh.normals.push_back(sommet/norm);
    }
}
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '-':
        setUnitSphere(unit_sphere, (--nX < 5) ? ++nX : nX, (--nY < 5) ? ++nY : nY);
        break;
