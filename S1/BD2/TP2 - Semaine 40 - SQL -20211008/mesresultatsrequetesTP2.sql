...
SELECT LIVRE.ISBN,TITRE, COUNT(*) AS "Nombre exemplaires" FROM EXEMPLAIRE 
JOIN LIVRE ON EXEMPLAIRE.ISBN=LIVRE.ISBN
WHERE CODE_PRET='EXCLU' AND LIVRE.ISBN IN (SELECT ISBN FROM EXEMPLAIRE JOIN EMPRUNT ON EXEMPLAIRE.NUMERO=EMPRUNT.NUM_EX GROUP BY ISBN HAVING COUNT(*)>=2)
